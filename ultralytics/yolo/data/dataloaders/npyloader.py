...
from .v5augmentations import (classify_albumentations, classify_transforms, copy_paste,
                              letterbox, mixup)

# augment_hsv is not used here as channels are not necessary RGB

from .npyaugmentations import (Albumentations, random_perspective)

#from .v5loader import LoadImagesAndLabels
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                elif p.is_file():  # file - DAMIR POGLEDATI OVO (mislim da je OK)
                    with open(p) as t:
            ...
            self.im_files = sorted(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in MAT_FORMATS)
        ...
        # Check cache
        self.label_files = npy2label_paths(self.im_files)  # labels
        # STARO - cache path: labels.cache
        # cache_path = (p if p.is_file() else Path(self.label_files[0]).parent).with_suffix('.cache')
        # DAMIR - cache path prema imenu (prvog) direktorija sa slikama
        ...
        self.label_files = npy2label_paths(cache.keys())  # update
            ...
            with ThreadPool(NUM_THREADS) as pool:
                ...
                for i, x in pbar:
                    if cache_images == 'disk':
                        pass # Do nothong, images are already npy files
                    else:  # 'ram'
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(verify_npy_label, zip(self.im_files, self.label_files, repeat(prefix), repeat(self.nchannels)))
